...
class serial {
    public:
        ...
        struct RegisterRef {
            ...
            RegisterRef& operator=(uint8_t value) {
                ports::outb(address, value);
                return *this;
            }

            operator uint8_t() const volatile {
                return ports::inb(address);
            }

            uint8_t operator&(int mask) const volatile {
                return static_cast<uint8_t>(*this) & mask;
            }
        };
        ...
        struct Port {
            ...
            inline RegisterRef operator[](Reg reg) const {
                return RegisterRef{ static_cast<uint16_t>(address + static_cast<uint8_t>(reg)) };
            }
        ...
        static void init() {
            Port* ports[] = { &COM1, &COM2, &COM3, &COM4, &COM5, &COM6, &COM7, &COM8 };

            for (auto* port : ports)
                init(*port, Baud115200);
        }

        static void init(Port& port, BaudRate baud) {
        ...
        static void set_interrupts(Port& port, bool enable) {
