...
void Heap::create(uint32_t start, uint32_t size, uint32_t max, uint16_t perms) {
	...
	assert(start % mm::pmm::PAGE_SIZE == 0);
	assert(end % mm::pmm::PAGE_SIZE == 0);
	...
	uint32_t phys = mm::pmm::alloc_pages(size / mm::pmm::PAGE_SIZE);
	mm::vmm::map_pages(start, phys, size / mm::pmm::PAGE_SIZE, this->perms | mm::Flags::Present);
...
void Heap::expand(size_t newSize) {
	...
	if (newSize % mm::pmm::PAGE_SIZE) {
		newSize &= -mm::pmm::PAGE_SIZE;
		newSize += mm::pmm::PAGE_SIZE;
	}
	...
	mm::vmm::map_pages(oldEnd, phys, size / mm::pmm::PAGE_SIZE, this->perms | mm::Flags::Present);
}
...
size_t Heap::contract(size_t newSize) {
	...
	if (phys == 0xFFFFFFFF)
		serial::printf("[heap] trying to free unmapped memory! addr: 0x%08x\n", this->endAddr);

	mm::vmm::unmap_pages(this->endAddr, size / mm::pmm::PAGE_SIZE);
	mm::pmm::free_pages(phys, size / mm::pmm::PAGE_SIZE);
...
_Lassert:
	kutils::panic("(alloc) Memory Corrupt");
	return nullptr;
}
...
_Lassert2:
	insertChunk(block);
_Lassert:
	kutils::panic("(free) Memory Corrupt");
}

void* Heap::palignedAlloc(size_t size) {
	size_t nb = request2size(size);
	size_t req = nb + mm::pmm::PAGE_SIZE + MIN_CHUNK_SIZE - CHUNK_OVERHEAD;

	void* mem = alloc(req);
	if (mem) {
		HeapChunk_t* chunk = mem2chunk(mem);
		if ((size_t)mem % mm::pmm::PAGE_SIZE) {
			char* br = (char*)mem2chunk(((size_t)mem & -mm::pmm::PAGE_SIZE) + mm::pmm::PAGE_SIZE);
			char* pos = ((size_t)(br - (char*)(chunk)) >= MIN_CHUNK_SIZE) ? br : br + mm::pmm::PAGE_SIZE;
		...
		assert(((size_t)mem % mm::pmm::PAGE_SIZE) == 0);
		assert(cinuse(chunk));
	}
